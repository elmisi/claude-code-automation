...
This repository packages Claude Code automation plugins, plus Codex support for `plan-cycle` and `refactor-discovery`. Core plugin assets live under `plugin/`: `schemas/` are the source of truth, `templates/` provide starting files, `skills/` contains `SKILL.md` workflows, `scripts/` contains validators and helpers, and `docs/` holds reference material. Additional plugins are split into `plugin-develop-cycle/`, `plugin-takeaway/`, `plugins/plan-cycle/`, and `plugins/refactor-discovery/`. Tests live in `tests/`, with fixtures in `tests/fixtures/` and runners in `tests/scripts/`.
...
./plugin/scripts/validate-config.sh skill ./tests/fixtures/skill-auto.md
```
