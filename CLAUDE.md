...
- **Hook events**: 33 valid events (`PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PostToolBatch`, `SessionStart`, `SessionEnd`, `UserPromptSubmit`, `UserPromptExpansion`, `PermissionRequest`, `PermissionDenied`, `Notification`, `MessageDisplay`, `Stop`, `StopFailure`, `PreCompact`, `PostCompact`, `PreModelSwitch`, `PostModelSwitch`, `SubagentStart`, `SubagentStop`, `TeammateIdle`, `TaskCreated`, `TaskCompleted`, `ConfigChange`, `InstructionsLoaded`, `WorktreeCreate`, `WorktreeRemove`, `Elicitation`, `ElicitationResult`, `CwdChanged`, `DirectoryAdded`, `FileChanged`, `Setup`). `MessageDisplay` fires while assistant text streams to the user — non-blocking, no matcher, can replace shown text via `hookSpecificOutput.displayContent` (display only). `DirectoryAdded` fires after `/add-dir` or the SDK `register_repo_root` request (matchers `slash_command`/`register_repo_root`) and cannot block — the directory is already added. `PreModelSwitch`/`PostModelSwitch` (v2.1.251+) match on the *canonical* name of the target model, so `claude-opus-5` covers every alias and dated ID; `PreModelSwitch` can block, `PostModelSwitch` cannot. NEVER use `PreCommit`, `PostCommit`, `PreBash`, `PostBash`, `BeforeToolUse`, `AfterToolUse` — they don't exist.
...
- **Hook handler types**: `command` (shell script), `http` (POST to URL), `mcp_tool` (call a tool on a connected MCP server), `prompt` (single-turn LLM), `agent` (multi-turn LLM). Support is **not** uniform — see `eventHandlerSupport` in `schemas/hooks.json`: every event takes `command` and `mcp_tool`; `http` everywhere except `SessionStart`/`Setup`; `prompt`/`agent` only on the 13 tool, prompt and stop events. Fields: `if` (permission rule syntax filter), `async`, `timeout`, `statusMessage`, `model`, `once`, `shell` (`bash`/`powershell`). HTTP type adds: `url`, `headers`, `allowedEnvVars`. `mcp_tool` adds: `server`, `tool`, `input`.
